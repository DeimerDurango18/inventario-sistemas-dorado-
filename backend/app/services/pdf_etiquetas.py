...
def _insertar_etiqueta(c: canvas.Canvas, x: float, y: float, fila) -> None:
    ...
    c.drawString(x + 5, y - 8, folio)
    c.drawImage(qr, x + LABEL_W - 27 * mm, y - 37 * mm, width=27 * mm, height=27 * mm)

    c.setFont("Helvetica-Bold", 9)
    c.drawString(x + 5, y - 20, f"{marca} {modelo}")
    c.setFont("Helvetica", 8)
    c.drawString(x + 5, y - 30, f"SERIE: {serie or 'N/D'}")
    c.drawString(x + 5, y - 38, f"ESTADO: {estado.upper()}")
    c.drawString(x + 5, y - 46, f"UBIC.: {ubicacion or '—'}")
